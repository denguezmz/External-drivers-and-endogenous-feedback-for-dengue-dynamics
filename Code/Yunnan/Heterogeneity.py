...
class ODENN(nn.Module):
    def __init__(self):
        ...
        self.init_res = []
        self.init_sym = []
        self.Nh = torch.tensor([47140000])
        self.S0 = torch.tensor([47140000])
        ...
        self.rate1 = torch.nn.Parameter(torch.tensor([20.4094], dtype=torch.float32), requires_grad=True)
        self.rate2 = torch.nn.Parameter(torch.tensor([1.3000], dtype=torch.float32), requires_grad=True)
        self.rate3 = torch.nn.Parameter(torch.tensor([9.1214], dtype=torch.float32), requires_grad=True)
        self.rateS0 = torch.nn.Parameter(torch.tensor([7.7098], dtype=torch.float32), requires_grad=True)

        self.res0 = torch.tensor([0])
        self.M0 = torch.tensor([0])
        self.N0 = torch.tensor([0])

        self.gamma = torch.nn.Parameter(torch.tensor([0.1100], dtype=torch.float32), requires_grad=True)
        self.gammaimp = torch.nn.Parameter(torch.tensor([0.03], dtype=torch.float32), requires_grad=True)
        self.U = torch.nn.Parameter(torch.tensor([0.01], dtype=torch.float32), requires_grad=True)
        self.detah = torch.nn.Parameter(torch.tensor([0.1022], dtype=torch.float32), requires_grad=True)
        self.rate1 = torch.nn.Parameter(torch.tensor([21.2691], dtype=torch.float32), requires_grad=True)
        self.rate2 = torch.nn.Parameter(torch.tensor([1.3000], dtype=torch.float32), requires_grad=True)
        self.rate3 = torch.nn.Parameter(torch.tensor([9.1214], dtype=torch.float32), requires_grad=True)


    def clamp_parameters(self):
        self.gamma.data = F.hardtanh(self.gamma, min_val=0.0714, max_val=0.11)
        self.gammaimp.data = F.hardtanh(self.gammaimp, min_val=0.01, max_val=0.03)
        self.U.data = F.hardtanh(self.U, min_val=0.0100, max_val=0.1740)
        self.detah.data = F.hardtanh(self.detah, min_val=0.100, max_val=0.20)
...
def train_model(model, optimizer, epochs):
    ...
    for epoch in range(epochs):
        optimizer.zero_grad()
        try:
            sol_pred, sol_res = model(num_steps, mymu[-730:], mydp[-730:], myda[-730:], myborn[-730:], AR[-730:],
                                      Import[-730:], T[-730:])
            ...
            M = sol_res[1:, 3]
            # myloss = loss_fn(H_pred_sum, Local_sum) +  0.5 *loss_fn(H_pred_sum[indices4], Local_sum[indices4])
            myloss = custom_loss(H_pred_sum, Local_sum, weight_above_100=1.5, weight_below_100=1.0)
            if torch.isnan(myloss):
                raise RuntimeError("NaN loss encountered")
            last_valid_state = {name: param.clone() for name, param in model.named_parameters()}
            last_valid_loss = myloss.item()
            ...
            if epoch % 20 == 0:
                ...
                ax1.scatter(t, Local_sum.detach().numpy(), color=color3, label='Exact Data Points', s=10,
                            edgecolor='black')
    ...
    if nan_encountered:
        if last_valid_state is not None:
            print("\nRestoring last valid parameters (before NaN occurred)...")
            print(f"Last valid loss: {last_valid_loss}")

            for name, param in model.named_parameters():
                param.data.copy_(last_valid_state[name])
            print("\nLast valid parameters:")
            return {name: param.data for name, param in model.named_parameters()}
        else:
            print("No valid parameters found (NaN occurred on first epoch).")
            return None
    else:
        print("Training completed successfully without NaN values.")
        return None
...
#     # {'params': [model.rateIp0], 'lr': 0.000000001},
    {'params': model.rateS0, 'lr': 0.01},
    {'params': [model.detah,model.U], 'lr': 0.01},
    {'params': [model.gamma,model.gammaimp,], 'lr': 0.01},
    # {'params': [model.rate3], 'lr': 0.01},
    {'params': [model.rate1,], 'lr': 0.01},
    {'params': [model.rate2, ], 'lr': 0.01},
])
...
df.to_excel('./result/Yunan_heterogeneity.xlsx', index=False, engine='openpyxl')
