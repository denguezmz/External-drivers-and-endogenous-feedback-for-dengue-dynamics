...
class ODENN(nn.Module):
    def __init__(self):
        ...
        self.rateS0 = torch.nn.Parameter(torch.tensor([7.0627], dtype=torch.float32), requires_grad=True)
        self.res0 = torch.tensor([0])
        self.M0 = torch.tensor([0])
        self.N0 = torch.tensor([0])
        self.gamma = torch.nn.Parameter(torch.tensor([0.0729], dtype=torch.float32), requires_grad=True)
        self.gammaimp = torch.nn.Parameter(torch.tensor([0.01], dtype=torch.float32), requires_grad=True)
        self.U = torch.nn.Parameter(torch.tensor([0.0439], dtype=torch.float32), requires_grad=True)
        self.detah = torch.nn.Parameter(torch.tensor([0.17], dtype=torch.float32), requires_grad=True)
        # self.beta0 = torch.nn.Parameter(torch.tensor([1.0705], dtype=torch.float32), requires_grad=True)
        ...
        # self.SpK = torch.tensor([5.0e+07])]
        # self.rate1 = torch.nn.Parameter(torch.tensor([8.3242], dtype=torch.float32), requires_grad=True)
        # self.rate2 = torch.nn.Parameter(torch.tensor([1.5250], dtype=torch.float32), requires_grad=True)
        # self.rate3 = torch.nn.Parameter(torch.tensor(10.7203, dtype=torch.float32), requires_grad=True)
        self.rate1 = torch.nn.Parameter(torch.tensor([6.9093], dtype=torch.float32), requires_grad=True)
        self.rate2 = torch.nn.Parameter(torch.tensor([1.4544], dtype=torch.float32), requires_grad=True)
        # self.rate3 = torch.nn.Parameter(torch.tensor([9.1428], dtype=torch.float32), requires_grad=True)
    ...
    def clamp_parameters(self):
        ...
        self.U.data = F.hardtanh(self.U, min_val=0.0140, max_val=0.1740)
        self.detah.data = F.hardtanh(self.detah, min_val=0.100, max_val=0.17)
...
def train_model(model, optimizer, epochs):
    ...
    for epoch in range(epochs):
        ...
        # myloss = custom_loss(H_pred_sum, Local_sum, weight_above_100=2.0, weight_below_100=1.0)
...
optimizer = torch.optim.Adam([
    ...
    {'params': [model.rate3], 'lr': 0.0001},
    {'params': [model.rate1,], 'lr': 0.001},
    {'params': [model.rate2, ], 'lr': 0.001},
])
# train_model(model, optimizer, epochs=4000)
...
df.to_excel('./result/Fujian_heterogeneity.xlsx', index=False, engine='openpyxl')