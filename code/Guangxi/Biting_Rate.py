...
class ODENN(nn.Module):
    def __init__(self):
        ...
        self.rateS0 = torch.nn.Parameter(torch.tensor([7.0429], dtype=torch.float32), requires_grad=True)
        self.gamma = torch.nn.Parameter(torch.tensor([0.0805], dtype=torch.float32), requires_grad=True)
        self.U = torch.nn.Parameter(torch.tensor([0.0531], dtype=torch.float32),
                                    requires_grad=True)  # 0.9944  0.014-0.174
        self.detah = torch.nn.Parameter(torch.tensor([0.1446], dtype=torch.float32), requires_grad=True)
        self.gammaimp = torch.nn.Parameter(torch.tensor([0.0300], dtype=torch.float32), requires_grad=True)
        self.rate1 = torch.nn.Parameter(torch.tensor([0.1027], dtype=torch.float32), requires_grad=True)
        self.rate2 = torch.nn.Parameter(torch.tensor([0.1075], dtype=torch.float32), requires_grad=True)
        self.rate3 = torch.nn.Parameter(torch.tensor([89.3861], dtype=torch.float32), requires_grad=True)
        self.rate4 = torch.nn.Parameter(torch.tensor([0.0011], dtype=torch.float32), requires_grad=True)
   # self.rate5 = torch.nn.Parameter(torch.tensor([0.2144], dtype=torch.float32), requires_grad=True)
    ...
    def forward(self, num_steps, mymu, mydp, myda, myborn, AR, Import, T):
        ...
        for step in range(num_steps):
            ...
            x0 = T[step] / 35
            x1 = (h[2] + h[4]) / 1000
            bt = torch.clamp((self.rate1 * x0 - self.rate2) / (1 + self.rate3 * x1 * x1) - self.rate4 * x1 + self.rate5,
                             min=0.0)
...
def train_model(model, optimizer,epochs):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0:
            ...
            I_range = np.linspace(0 / 1000, 600 / 1000, 100)
...
optimizer = torch.optim.Adam([
    {'params': model.detah, 'lr': 0.00001},
    {'params': model.U, 'lr': 0.00001},
    {'params': model.gamma, 'lr': 0.00001},
    {'params': model.gammaimp, 'lr': 0.00001},
    # {'params': model.rateS0, 'lr': 0.0001},
    {'params': model.rate4, 'lr': 0.00001},
    {'params': [model.rate2], 'lr': 0.00001},
    {'params': [model.rate3], 'lr': 0.00001},
], lr=0.000001)
...
df.to_excel('./result/Guangxi_biting_rate.xlsx', index=False, engine='openpyxl')
