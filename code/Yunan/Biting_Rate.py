...
class ODENN(nn.Module):
    def __init__(self):
        ...
        # self.rateS0 = torch.nn.Parameter(torch.tensor([6.3506], dtype=torch.float32), requires_grad=True)
        self.rateS0 = torch.nn.Parameter(torch.tensor([7.5615], dtype=torch.float32), requires_grad=True)
        # self.gamma = torch.nn.Parameter(torch.tensor([0.0714], dtype=torch.float32), requires_grad=True)
        self.gamma = torch.nn.Parameter(torch.tensor([0.1233], dtype=torch.float32), requires_grad=True)
        # self.myrate = torch.nn.Parameter(torch.tensor([-0.0598], dtype=torch.float32), requires_grad=True)
        # self.myrate2 = torch.nn.Parameter(torch.tensor([0.9311], dtype=torch.float32), requires_grad=True)
        self.U = torch.nn.Parameter(torch.tensor([0.0100], dtype=torch.float32),requires_grad=True)
        self.detah = torch.nn.Parameter(torch.tensor([0.1012], dtype=torch.float32), requires_grad=True)
        self.gammaimp = torch.nn.Parameter(torch.tensor([0.030], dtype=torch.float32), requires_grad=True)
        self.rate1 = torch.nn.Parameter(torch.tensor([0.1027], dtype=torch.float32), requires_grad=True)
        self.rate2 = torch.nn.Parameter(torch.tensor([0.1229], dtype=torch.float32), requires_grad=True)
        self.rate3 = torch.nn.Parameter(torch.tensor([81.500], dtype=torch.float32), requires_grad=True)
        self.rate4 = torch.nn.Parameter(torch.tensor([0.0220], dtype=torch.float32), requires_grad=True)
        # self.rate5 = torch.nn.Parameter(torch.tensor([0.1984], dtype=torch.float32), requires_grad=True)

    def clamp_parameters(self):
        self.gamma.data = F.hardtanh(self.gamma, min_val=0.0714, max_val=0.1233)
        self.gammaimp.data = F.hardtanh(self.gammaimp, min_val=0.01, max_val=0.03)
        self.U.data = F.hardtanh(self.U, min_val=0.0100, max_val=0.1240)
        ...
        self.rateS0.data = F.hardtanh(self.rateS0, min_val=7.5615, max_val=10)
...
def train_model(model, optimizer,epochs):
    ...
    for epoch in range(epochs):
        ...
        # myloss = loss(H_pred_sum, Local_sum) + 0.5 * loss(H_pred_sum[indices4], Local_sum[indices4])
        ...
        if epoch % 20 == 0:
            ...
            for i in range(X.shape[0]):
                for j in range(X.shape[1]):
                    x0 = X[i, j]
                    x1 = Y[i, j]
                    # Z[i, j] =model.rate1 - 1/((1-model.rate2*x1)*(model.rate3*x0+model.rate4*x1+model.rate5))
                    Z[i, j] = (model.rate1 * x0 - model.rate2) / (
                                model.rate3 * x1 * x1 + 1) - model.rate4 * x1 + model.rate5
            fig, ax = plt.subplots(figsize=(10, 4))
            contour = ax.contourf(X, Y, Z, 50, cmap='coolwarm', alpha=0.8,
                                  levels=np.linspace(0.09, 0.21, 50),
                                  vmin=0.09,  # Enforce min
                                  vmax=0.21  # Enforce max
                                  )
...
df.to_excel('./result/Yunan_biting_rate.xlsx', index=False, engine='openpyxl')
